...
/* ============================================================
   MISSÃO PMMG V7.3.2 — SEU PROGRESSO (IMPLEMENTAÇÃO SEGURA)
   O HTML é estático. Este código apenas atualiza números.
   ============================================================ */
function updateProgressV732(){
  let hist=[];
  try{
    hist=JSON.parse(localStorage.getItem("pmmg_sim_history_v72")||"[]");
    if(!Array.isArray(hist)) hist=[];
  }catch(e){hist=[]}
  const sims=hist.length;
  const avg=sims?Math.round(hist.reduce((sum,x)=>sum+(Number(x.pct)||0),0)/sims):0;
  const best=sims?Math.max(...hist.map(x=>Number(x.pct)||0)):0;
  ...
  put("v732Sims",sims);
  put("v732Best",best+"%");
  put("v732Errors",errors);
}
window.updateProgressV732=updateProgressV732;

document.addEventListener("DOMContentLoaded",()=>setTimeout(updateProgressV732,150));

/* Atualiza os números quando o painel principal já estiver sendo atualizado,
   sem alterar navegação, classes de tela ou estrutura do DOM. */
const updateDashboardV732Base=window.updateDashboard;
if(typeof updateDashboardV732Base==="function"){
  window.updateDashboard=function(){
    const result=updateDashboardV732Base.apply(this,arguments);
    try{updateProgressV732()}catch(e){}
    return result;
  };
