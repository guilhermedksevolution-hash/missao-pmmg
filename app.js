...
/* ============================================================
   MISSÃO PMMG V7.3.0 — ESTATÍSTICAS COMPACTAS
   ============================================================ */
function v73CountAnswered(){
  let n=0;
  try{
    const errors=JSON.parse(localStorage.getItem("pmmg_errors")||"[]");
    const hist=JSON.parse(localStorage.getItem("pmmg_sim_history_v72")||"[]");
    n += Array.isArray(errors)?errors.length:0;
    if(Array.isArray(hist)) n += hist.reduce((a,x)=>a+(Number(x.total)||0),0);
  }catch(e){}
  return n;
}
function v73Stats(){
  let hist=[];
  try{hist=JSON.parse(localStorage.getItem("pmmg_sim_history_v72")||"[]")}catch(e){}
  if(!Array.isArray(hist))hist=[];
  const sims=hist.length;
  const avg=sims?Math.round(hist.reduce((a,x)=>a+(Number(x.pct)||0),0)/sims):0;
  const answered=v73CountAnswered();
  let best=sims?Math.max(...hist.map(x=>Number(x.pct)||0)):0;
  return {sims,avg,answered,best};
}
function v73MountStats(){
  const host=document.querySelector("#trainingHubV53, #trainHubV53");
  if(!host || document.getElementById("v73StatsPanel"))return;
  const s=v73Stats();
  const panel=document.createElement("section");
  panel.id="v73StatsPanel";
  panel.innerHTML=`
    <div class="v73-head"><b>Seu progresso</b><button onclick="openSimulationHistoryV72()">Ver evolução ›</button></div>
    <div class="v73-stat-grid">
      <article><span>🎯</span><strong>${s.avg}%</strong><small>Desempenho médio nos simulados</small></article>
      <article><span>✅</span><strong>${s.answered}</strong><small>Questões registradas</small></article>
      <article><span>📊</span><strong>${s.sims}</strong><small>Simulados realizados</small></article>
      <article><span>🏆</span><strong>${s.best}%</strong><small>Melhor simulado</small></article>
    </div>`;
  host.appendChild(panel);
}
document.addEventListener("DOMContentLoaded",()=>setTimeout(v73MountStats,250));
